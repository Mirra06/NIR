...
bl=1e8; % Длина информационных символов
% bl=8; % Длина информационных символов
N=bl/2; % Длина блока 
for cntr=1:length(snr1)
    % info_bits = [1 0 1 0 1 0 1 0];
    ...
    %  y1 = awgn(mod_code_bit1, snr1(cntr), 'measured');
     Nsyms = bl; %Количество символов
     M = 2; %Порядок модуляции
     SNR = snr1(cntr);
     Esig = sum(mod_code_bit0 .* conj(mod_code_bit0)); %Энергия сигнала
     Esym = Esig/Nsyms; %Энергия символа
     Eb = 2*Esym/log2(M); %Энергия бита
     N0 = Eb / 10^(SNR/10); %Энергия шума
     noise_0 = sqrt(N0/2) * (randn(1, length(mod_code_bit0)) + 1j*randn(1, length(mod_code_bit0)));
     noise_1 = sqrt(N0/2) * (randn(1, length(mod_code_bit1)) + 1j*randn(1, length(mod_code_bit1)));
     y0 = mod_code_bit0 + noise_0'; %Добавление шума
     y1 = mod_code_bit1 + noise_1';
    % Демодулятор
     y00=de_bpsk(y0);
     y11=de_bpsk(y1);
