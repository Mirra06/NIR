...
%% Канал с шумом AWGN
 [y0, N0_0] = noise_awgn(mod_code_bit0, snr3(cntr), M, bl); % Добавление шума
 [y1, N0_1] = noise_awgn(mod_code_bit1, snr3(cntr), M, bl);
%% BPSK демодулятор
...
for p = 1:N:bl
    ...
    s = 1;
        for j = 1:2:2*len
            metric(j,i) = ((y00(i+p-1)*out_code_comb_1(m,1)) + (y11(i+p-1)*out_code_comb_1(m,2))); % Метрика ветви
            m = m+1;
        end
        m = 1;
        for k = 1:4:2*len              
            distance(m,i+1) = max(distance(s,i)+metric(k,i),distance(s+1,i)+metric(k+2,i)); % Минимальная метрика
            s = s+2;
            m = m+1;
        end     
    ...
    s = 1;
        for j = 2:2:2*len
            metric(j,i) = ((y00(i+p-1)*out_code_comb_2(m,1)) + (y11(i+p-1)*out_code_comb_2(m,2))); % Метрика ветви
            m = m+1;
        end
        m = 1;
        for k = 2:4:2*len
            distance(m+len/2,i+1) = max(distance(s,i)+metric(k,i),distance(s+1,i)+metric(k+2,i)); % Минимальная метрика
            s = s+2;
            m = m+1;
        end 
    end

[~,state]=max(distance(:,N+1)); % Выбираем минимальную метрику в последнем узле
for h = N:-1:1
    [state,decoded_bit]=prev_stage_corr(state,distance(:,h),metric(:,h),len);
    decoded_bit_final(h+p-1)=decoded_bit; % Декодированная последовательность
end
end
%%
    [ERR1,ratio3] = biterr(info_bits,decoded_bit_final);
    per3(cntr)=ratio3;
end
