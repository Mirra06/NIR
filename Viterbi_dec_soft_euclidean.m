...
tb = 15;
% Добавление шума
 y0 = awgn(mod_code_bit0, snr3(cntr), 'measured');
 y1 = awgn(mod_code_bit1, snr3(cntr), 'measured');
% Демодулятор
 y00=de_bpsk(y0);
 y11=de_bpsk(y1);
...
for i=1:N
 metric(1,i) = sqrt(sum(([y00(i+p-1),y11(i+p-1)]-[1,1]).^2)); % Метрика ветви от 00 к 00
 metric(2,i) = sqrt(sum(([y00(i+p-1),y11(i+p-1)]-[-1,-1]).^2)); % Метрика ветви от 00 к 10
 metric(3,i) = sqrt(sum(([y00(i+p-1),y11(i+p-1)]-[-1,1]).^2)); % Метрика ветви от 10 к 01
 metric(4,i) = sqrt(sum(([y00(i+p-1),y11(i+p-1)]-[1,-1]).^2)); % Метрика ветви от 10 к 11
 metric(5,i) = sqrt(sum(([y00(i+p-1),y11(i+p-1)]-[-1,-1]).^2)); % Метрика ветви от 01 к 00
 metric(6,i) = sqrt(sum(([y00(i+p-1),y11(i+p-1)]-[1,1]).^2)); % Метрика ветви от 01 к 10
 metric(7,i) = sqrt(sum(([y00(i+p-1),y11(i+p-1)]-[1,-1]).^2)); % Метрика ветви от 11 к 01
 metric(8,i) = sqrt(sum(([y00(i+p-1),y11(i+p-1)]-[-1,1]).^2)); % Метрика ветви от 11 к 11
distance(1,i+1)=min(distance(1,i)+metric(1,i),distance(3,i)+metric(5,i)); % Минимальная метрика в 00
